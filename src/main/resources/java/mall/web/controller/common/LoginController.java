...
public class LoginController extends DefaultController{
	...
	*/
	...
	public ModelAndView getLogin(@ModelAttribute TB_MBINFOXM memberInfo, Model model, HttpSession session) throws Exception {
		...
		String strEncrpytPasswd = util.kisaSha256(memberInfo.getMEMB_PW().trim());
		//strEncrpytPasswd = memberInfo.getMEMB_PW().trim();
		/*
		//일반유저 로그인제한
		...
		*/
		if (loginMember == null || !loginMember.getMEMB_PW().equals( strEncrpytPasswd )) {
		...
		} else if ("00000000002".equals(authCheck.getGROUP_CD()) || "00000000004".equals(authCheck.getGROUP_CD())) {
			// 세션 생성
			session.setAttribute("ADMUSER",  loginMember);
			mav.setViewName("redirect:" + "/adm/main");

		} else if ("00000000001".equals(authCheck.getGROUP_CD())) {
			// 세션 생성
			session.setAttribute("ADMUSER",  loginMember);
